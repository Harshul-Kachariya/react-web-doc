 ...
 const MarkdownPreview = lazy(() => import('./MarkdownPreview.js'));`}
          ...
          <div className="mt-5">
            ...
            <p className="text-xl mb-2 ">
              ...
              returns a Promise or another thenable (a Promise-like object with
              a then method). React will not call{" "}
              <span className="text-red-500"> load</span> until the first time
              you attempt to render the returned component. After React first
              calls
              <span className="text-red-500"> load </span>, it will wait for it
              to resolve, and then render the resolved value’s .default as a
              React component. Both the returned Promise and the Promise’s
              resolved value will be cached, so React will not call{" "}
              <span className="text-red-500"> load </span> more than once. If
              the Promise rejects, React will throw the rejection reason for the
   ...
   const MarkdownPreview = lazy(() => import('./MarkdownPreview.js'));`}
          ...
          <p className="text-xl mb-2 ">
            ...
            specify what should be displayed while it is loading. You can do
            this by wrapping the lazy component or any of its parents into a{" "}
            <span className="text-emerald-600 font-semibold">{`<Suspense>`}</span>{" "}
            boundary:
          </p>
        </div>
